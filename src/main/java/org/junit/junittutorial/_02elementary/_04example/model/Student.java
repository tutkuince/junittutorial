...
public class Student implements UniversityMember {
	...
	public void dropCourse(LecturerCourseRecord lecturerCourseRecord) {
		...
		final StudentCourseRecord studentCourseRecordWillBeDropped = studentCourseRecords.stream().filter(
				studentCourseRecord -> studentCourseRecord.getLecturerCourseRecord().equals(lecturerCourseRecord))
				.findAny().orElseThrow(() -> new IllegalArgumentException(
						"There is no student course record for given lecturer course record"));
	...
	public void addGrade(LecturerCourseRecord lecturerCourseRecord, StudentCourseRecord.Grade grade) {
		...
		final StudentCourseRecord studentCourseRecord1 = studentCourseRecords.stream().filter(
				studentCourseRecord -> studentCourseRecord.getLecturerCourseRecord().equals(lecturerCourseRecord))
				.findAny()
				.orElseThrow(() -> new IllegalArgumentException(new NoCourseFoundForStudentException(
	...
	public BigDecimal gpa() {
		...
		for (StudentCourseRecord studentCourseRecord : studentCourseRecords) {
			...
			weightedGpa = weightedGpa.add(
					BigDecimal.valueOf(courseCredit(studentCourseRecord)).multiply(courseGrade(studentCourseRecord)));
		}
